...
import { JWT_SECRET, APP_ENV } from '@/constants';
...
export function authenticate({ email, password }: { email: string; password: string }) {
  ...
  if (user?.password === hash(password)) {
    ...
    const encodedToken = JWT.encode({ id: user.id }, JWT_SECRET);
...
export function requireAuth(request: RestRequest) {
  try {
    ...
    const decodedToken = JWT.decode(encodedToken, JWT_SECRET);

    const user = db.user.findFirst({
      where: {
        id: {
          equals: decodedToken.id as string,
        },
      },
    });
